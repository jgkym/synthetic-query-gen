...
def corpus_to_examples(json_path: str, chunk_size: int = 512) -> List[Example]:
    ...
    documents = processor.process_corpus(documents=content, document_ids=document_ids, chunk_size=chunk_size)

    return [Example(source=doc['document_id'], content=doc['content']) for doc in documents]